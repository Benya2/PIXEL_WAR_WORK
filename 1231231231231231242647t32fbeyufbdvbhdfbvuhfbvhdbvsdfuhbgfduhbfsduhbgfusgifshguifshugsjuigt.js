...
import { getDatabase, ref, remove, onValue, set, onDisconnect } from "https://www.gstatic.com/firebasejs/11.2.0/firebase-database.js";
...
const MIN_SCALE = 0.005, MAX_SCALE = 6;
...
function snapToGrid(wx,wy){
  return [Math.floor(wx/gridCellSize)*gridCellSize, Math.floor(wy/gridCellSize)*gridCellSize];
}
...
function renderAll(){
  ...
  pixelsCache.forEach(d=>{
    if(d.color!=="#FFFFFF"){ctx.fillStyle=d.color; ctx.fillRect(d.x,d.y,gridCellSize,gridCellSize);}
  });
...
game.addEventListener('click', (e)=>{if(!isPanning&&e.button===0) placePixelWithHover();});
function startReload(){
  let t=reloadTime;
  reloadTimerEl.innerText=`Reload: ${t} sec`;
  const interval=setInterval(()=>{
    t--; if(t<=0){clearInterval(interval); canPlace=true; reloadTimerEl.innerText="Ready!";} else reloadTimerEl.innerText=`Reload: ${t} sec`;
  },1000);
}
...
authButton.addEventListener('click', async()=>{
  ...
  try{
    if(action==="1"){await signInWithEmailAndPassword(auth,email,pass); alert("Come in!");}
...
onAuthStateChanged(auth,user=>{
  if(user){
    ...
    trackOnlinePlayer(); // отслеживание онлайн
...
function trackOnlinePlayer(){
  ...
  set(userRef,{email:auth.currentUser.email,lastSeen:Date.now()});
...
teleportBtn.addEventListener('click',()=>{
  ...
  if(!value) return;
  const [xStr,yStr]=value.split(/\s+/), xCell=parseInt(xStr), yCell=parseInt(yStr);
  if(Number.isNaN(xCell)||Number.isNaN(yCell)) return alert("Введите корректные координаты X Y");
  const worldX=xCell*gridCellSize, worldY=yCell*gridCellSize;
  camX=worldX-(game.width/2)/scale;
  camY=worldY-(game.height/2)/scale;
  renderAll();
});

// ===== Admin: clear/add/remove pixels =====
function parseCoords(){
  ...
  value.split(",").forEach(part=>{
    ...
    for(let dx=0;dx<wCell;dx++){for(let dy=0;dy<hCell;dy++){
      ...
      if(px>=0 && py>=0 && px<=WORLD_W-gridCellSize && py<=WORLD_H-gridCellSize) markers.push([px,py]);
    }}
  });
  renderAll();
}
